...
const SmallChopsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const data = await menuService.getProductsByCategory('SMALL CHOPS');
        setProducts(data);
        setLoading(false);
      } catch (err) {
        console.error('Error fetching small chops products:', err);
        setError('Failed to load small chops products. Please try again later.');
        setLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading small chops products...</p>
          </div>
        ) : error ? (
