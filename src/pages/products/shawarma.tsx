...
const ShawarmaPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const data = await menuService.getProductsByCategory('SHAWARMA');
        setProducts(data);
        setLoading(false);
      } catch (err) {
        console.error('Error fetching shawarma products:', err);
        setError('Failed to load shawarma products. Please try again later.');
        setLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading shawarma products...</p>
          </div>
        ) : error ? (
