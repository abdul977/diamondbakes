...
const CakesPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const data = await menuService.getProductsByCategory('CAKES');
        setProducts(data);
        setLoading(false);
      } catch (err) {
        console.error('Error fetching cake products:', err);
        setError('Failed to load cake products. Please try again later.');
        setLoading(false);
      }
    };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600">Loading cake products...</p>
          </div>
        ) : error ? (
