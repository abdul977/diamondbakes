  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Use the decoded category name for display but encoded for API request
        const decodedCategoryName = decodeURIComponent(categoryName);
        const data = await menuService.getProductsByCategory(decodedCategoryName);
        setProducts(data);
        setLoading(false);
      } catch (err) {
        console.error(`Error fetching ${categoryName || ''} products:`, err);
        setError(`Failed to load ${(categoryName || '').toLowerCase()} products. Please try again later.`);
        setLoading(false);
      }
    };
