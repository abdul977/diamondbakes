// This script seeds FAQ data using the MongoDB MCP server
...
// Function to check if the collection exists and has data
...
async function clearCollection() {
  try {
    ...
    console.log('Collection cleared:', result);
    return true;
  } catch (error) {
...
async function insertDocument(document) {
  try {
    const result = await mongodb.create({
      database: 'diamondbakes',
      collectionName: 'faqcategories',
...
async function seedFAQData() {
  try {
    ...
    if (dataExists) {
      console.log('FAQ data already exists. Clearing collection...');
    ...
    for (const category of preparedData) {
    ...
    // If running in browser, show alert
