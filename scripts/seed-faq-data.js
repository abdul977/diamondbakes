// This script is meant to be run in the browser console when connected to the admin dashboard
// It will use the faqService to create the categories and questions
...
async function seedFAQData() {
  try {
    // Import the faqService
    const { faqService } = await import('../src/services/faqService.js');
    ...
    for (const category of faqData) {
      ...
      const newCategory = await faqService.createFAQCategory({
        name: category.name,
      ...
      for (const question of category.questions) {
        ...
        await faqService.addQuestion(newCategory.id, question);
      }
      
      console.log(`All questions added to category: ${category.name}`);
    }
