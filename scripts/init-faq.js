...
import FAQCategory from '../server/models/faqModel.js';
...
async function seedFAQData() {
  try {
    // Connect to MongoDB
    const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://abdul977:salis977@cluster0.s6mmj.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0';
    ...
    // Generate IDs and insert new data
    for (let i = 0; i < faqData.length; i++) {
      const categoryId = (i + 1).toString();
      const category = new FAQCategory({
        id: categoryId,
        name: faqData[i].name,
        order: faqData[i].order,
        questions: faqData[i].questions
      });
      await category.save();
    }
